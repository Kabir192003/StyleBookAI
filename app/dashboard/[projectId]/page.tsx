...
function LiveMockup({ project }: { project: Project }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-app-border p-6" style={{ background: bg }}>
      <span className="absolute right-4 top-4 rounded bg-app-heading px-2 py-1 font-mono text-[9px] font-bold text-pearl">
        LIVE
      </span>
      <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider" style={{ color: text, opacity: 0.6 }}>
        Live mockup
      </h3>
      <div className="rounded-xl p-6 shadow-app-sm" style={{ background: surface || "#fff" }}>
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      ...
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="app-eyebrow mb-1">Plate No. 01</p>
        ...
        <div className="flex flex-wrap items-center gap-2">
            ...
            <AnimatePresence>
              {exportOpen && (
                <motion.div
                  ...
                  className="absolute right-0 top-full z-20 mt-2 w-48 rounded-lg border border-glass-panel bg-app-glass-panel p-1.5 shadow-app-lg backdrop-blur-xl"
                >
          ...
          {!confirmingDelete ? (
            ...
            <div className="flex items-center gap-2 rounded-full border border-app-danger/30 bg-app-danger-soft px-3 py-1.5 text-sm text-app-danger">
              Delete this project?
              <button
                onClick={handleDelete}
                disabled={deleting}
                className="rounded-full bg-app-danger px-2.5 py-1 text-xs font-semibold text-pearl disabled:opacity-60"
              >
      ...
      {exportError && <p className="mb-4 text-xs text-app-danger">{exportError}</p>}
      {deleteError && <p className="mb-4 text-xs text-app-danger">{deleteError}</p>}

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1.3fr_1fr]">
        <div className="relative flex flex-col gap-6 overflow-hidden rounded-2xl border border-glass-panel bg-app-glass-panel p-6 shadow-app-md backdrop-blur-xl">
            ...
            <div className="flex flex-wrap gap-4">
              {project.colors.map((c) => (
                <div key={c.id + (c.role ?? "")} className="flex flex-col gap-1.5">
                  <div className="h-14 w-20 rounded-lg border border-app-border" style={{ background: c.hex }} />
