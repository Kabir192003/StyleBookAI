    ...
    <footer className="border-t border-app-border bg-app-bg">
      <div className="mx-auto flex max-w-7xl flex-col gap-6 px-6 py-10 md:flex-row md:items-center md:justify-between">
        <div className="flex flex-col gap-1">
          <Link href="/" className="font-geometric-sans text-base font-bold text-app-heading">
            StyleBook
          </Link>
        ...
        <nav className="flex flex-wrap gap-x-6 gap-y-2" aria-label="Footer">
          {LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-xs font-medium text-app-text-secondary transition-colors hover:text-app-heading"
            >
              {link.label}
            </Link>
          ))}
